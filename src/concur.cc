...
//std::time_t currtime = std::time({});
//char timeString[std::size("yyyy-mm-ddThh:mm:ssZ")];
//std::strftime(std::data(timeString), std::size(timeString),
//              "%FT%TZ", std::gmtime(&currtime));
ofstream e("outputs/" + to_string(chrono::system_clock::now().time_since_epoch().count()));
...
const int CONCUR = max(1u, std::thread::hardware_concurrency());
